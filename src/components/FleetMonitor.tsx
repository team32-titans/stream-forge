...
export const FleetMonitor: React.FC = () => {
  ...
  const [liveStatus, setLiveStatus] = useState<string>('Waiting for backend…');
      ...
      const load = async () => {
        try {
          ...
          if (!cancelled) {
            ...
            setLiveStatus(data.warning ? `Degraded: ${data.warning}` : `LIVE — ${data.count ?? 0} events from Kafka`);
          }
        } catch (e: any) {
          if (!cancelled) {
            setLiveEvents([]);
            setLiveStatus(`Unavailable — waiting for backend (${e?.message ?? 'fetch failed'})`);
          }
        }
      };
      load();
          ...
          <div className="mt-3 overflow-x-auto">
            <table className="w-full text-left font-mono text-xs">
              ...
              <tbody className="divide-y divide-[#1e293b]">
                {liveEvents.slice(0, 10).map((evt: any, idx: number) => (
                  <tr key={idx} className="text-slate-200">
                    <td className="py-2.5 text-orange-300 font-bold">{evt.truck_id ?? evt.truckId ?? '?'}</td>
                    <td className="py-2.5 text-slate-400">P{evt.partition ?? '?'}</td>
                    <td className="py-2.5">{evt.temperature ?? '?'}°C</td>
                    <td className="py-2.5 text-slate-400">{evt.timestamp ? new Date(evt.timestamp).toLocaleTimeString() : '?'}</td>
                  </tr>
                ))}
                {liveEvents.length === 0 && (
                  <tr><td colSpan={4} className="py-4 text-slate-500">No live events yet — {liveStatus}</td></tr>
                )}
              </tbody>
            </table>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="flex items-center justify-between border-b border-[#1e293b] pb-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-xs font-bold uppercase tracking-wider text-slate-200">
                Demo Ingested Telemetry Feed (Simulation)
              </h3>
            </div>
            <input
              type="text"
        ...
        <div className="bg-[#111827] border border-[#1e293b] rounded-2xl p-5 shadow-xl space-y-4 flex flex-col justify-between">
          ...
          <div className="pt-3 border-t border-[#1e293b] text-[10px] text-slate-400 font-mono flex items-center justify-between">
            <span className="font-sans">State Store: RocksDB MemTable (demo)</span>
            <span className="text-emerald-400 font-bold">Demo changelog mirror</span>
          </div>
        </div>
      </div>
      </>)}
