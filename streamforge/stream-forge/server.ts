...
async function startServer() {
  ...
  // Gracefully handle EADDRINUSE if port 3000 is briefly occupied during rapid restarts
  httpServer.on('error', (err: any) => {
    if (err.code === 'EADDRINUSE') {
      console.warn(`[StreamForge] Port ${PORT} busy, retrying in 1.2s...`);
      setTimeout(() => {
        try {
          httpServer.close();
        } catch {}
      ...
      console.error('[StreamForge] Server error:', err);
    }
  });
