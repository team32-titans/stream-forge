...
export const Member1Handbook: React.FC = () => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-[#111827] border border-[#1e293b] rounded-3xl p-6 shadow-xl space-y-3">
        <div className="flex items-center gap-2 text-indigo-400 font-bold text-xs uppercase tracking-widest">
          <Sparkles className="w-4 h-4 text-indigo-400" />
          Project Review Introduction Script (Elevator Pitch)
        </div>
        <blockquote className="bg-[#0a0c10] p-4 rounded-2xl border border-[#223348] text-xs text-slate-200 leading-relaxed font-sans italic">
          &ldquo;Hello everyone. In StreamForge, I am <strong>Member 1</strong>, responsible for the <strong>Core Stream Processing & Stateful Engine</strong>. My mission was to build a Python distributed streaming pipeline targeting <strong>100,000 events/second (target — measured baseline in docs/BENCHMARK.md)</strong> for 50,000 IoT refrigerated trucks. I engineered the <strong>5-minute rolling window aggregation engine</strong> using Welford's incremental statistical algorithm, integrated the <strong>embedded RocksDB state store</strong> with changelog replication, and built the <strong>effectively-once failover protocol (at-least-once + idempotent state)</strong> that replays changelog state when a worker node crashes.&rdquo;
        </blockquote>
      </div>

      {/* Week-by-Week Deep Dive Modules */}
      <div className="space-y-3">
        <h3 className="text-xs font-bold uppercase tracking-widest text-white flex items-center gap-2 px-1">
        ...
        <div className="space-y-3">
          {MEMBER1_CURRICULUM.map((topic) => {
            ...
            return (
              <div
                key={topic.id}
                className="bg-[#111827] border border-[#1e293b] rounded-3xl p-5 shadow-xl space-y-4 transition"
              >
                {/* Topic Header */}
                <div
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] px-2.5 py-0.5 rounded-full bg-[#16202e] text-indigo-300 font-mono border border-[#223348] hidden sm:inline uppercase tracking-wider">
                      {topic.roleFocus}
                    </span>
                    {isExpanded ? (
                ...
                {/* Expanded Content */}
                {isExpanded && (
                  <div className="pt-4 border-t border-[#223348] space-y-4 text-xs">
                    {/* Deep Dive Text */}
                    <div className="bg-[#0a0c10] p-4 rounded-2xl border border-[#223348] text-slate-300 leading-relaxed whitespace-pre-line font-sans">
                      {topic.deepDive}
                    </div>
                    ...
                    <div className="space-y-2.5 pt-2">
                      <div className="font-bold text-white text-xs flex items-center gap-1.5 uppercase tracking-wider">
                      ...
                      {topic.vivaQuestions.map((viva, vIdx) => (
                        <div
                          key={vIdx}
                          className="bg-[#0a0c10] p-4 rounded-2xl border border-[#223348] space-y-2"
                        >
      ...
      <div className="bg-[#111827] border border-[#1e293b] rounded-3xl p-6 shadow-xl space-y-4">
        <h3 className="text-xs font-bold uppercase tracking-widest text-white flex items-center gap-2 border-b border-[#223348] pb-3">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs font-sans">
            <thead>
              <tr className="border-b border-[#223348] text-[10px] uppercase text-slate-400 font-mono tracking-wider">
            ...
            <tbody className="divide-y divide-[#223348] text-slate-300">
              <tr>
