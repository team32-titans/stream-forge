...
def run_server():
    try:
        server = ReusableHTTPServer(("0.0.0.0", PORT), PythonEngineHandler)
        print(f"⚡ [StreamForge] Python Engine FastAPI/ASGI Bridge listening on http://0.0.0.0:{PORT}")
        server.serve_forever()
    except OSError as e:
        if getattr(e, 'errno', None) == 98:
            # Address already in use by previous worker
            print(f"⚡ [StreamForge] Python Engine port {PORT} already active.")
            sys.exit(0)
        raise
